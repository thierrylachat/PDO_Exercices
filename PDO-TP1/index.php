...
<body>
	<div class="container">
		...
		<div class="p-3 w-100 bg-dark rounded-lg shadow d-flex justify-content-between align-items-center">
			...
			<!-- Création du menu déroulant permettant de filtrer les utilisateurs par service. -->
			<form action="" method="GET">
				<div class="d-flex">
					<select class="form-control" name="filter" id="filter">
						...
						<option
							<?= (isset($_GET['filter']) && $_GET['filter'] == $service->services_id) ? 'selected' : ''; ?>
							value="<?= $service->services_id; ?>"><?= $service->name; ?></option>
						<?php endforeach; ?>
					</select>
				</div>
			</form>

			<!-- Création du bouton permettant d'ajouter un nouvel utilisateur. -->
			<div>
		...
		<!-- Création de cards pour chaque utilisateur. -->
			...
			<div class="col-lg-4 col-md-6">
				<div class="my-1 card border-dark">
					<div class="card-body">
						...
						<!-- Ajout des boutons supprimer et annuler sur chaque card. -->
						<div class="w-100 d-flex justify-content-end">
							<form action="" method="POST">
								<!-- Création d'un input hidden qui récupère l'id du user. -->
								<input type="hidden" name="users_id" value="<?= $user->users_id; ?>">
								<button class="btn btn-sm btn-secondary stop d-none" type="button"
									data-id="<?= $user->users_id; ?>">Annuler</button>
								<button class="btn btn-sm btn-danger delete" type="button"
									data-delete="<?= $user->users_id; ?>">Supprimer</button>
							</form>
						</div>
